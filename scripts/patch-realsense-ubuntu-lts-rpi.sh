...
then
	...
	echo -e "\e[32mFetching Ubuntu LTS source \e[47m${LINUX_BRANCH}\e[0m \e[32m to the local kernel sources folder\e[0m"
	cd ..
	rm -rf ${kernel_name}
	sudo rm -rf tmp
	mkdir tmp
	cd tmp
	apt-get source linux-image-${LINUX_BRANCH}
	mv linux-raspi-5.4-5.4.0 ../${kernel_name}
	cd ..
	sudo rm -rf tmp
	...
	then 
		...
		patch -p1 < ../scripts/realsense-camera-formats-${ubuntu_codename}-rpi.patch
...
sudo make -j -C $KBASE M=drivers/media/usb/uvc
echo -e "\e[32mCompiling accelerometer and gyro modules\e[0m"
sudo make -j -C $KBASE M=drivers/iio/accel
sudo make -j -C $KBASE M=drivers/iio/gyro
echo -e "\e[32mCompiling v4l2-core modules\e[0m"
sudo make -j -C $KBASE M=drivers/media/v4l2-core
if [[ ( $xhci_patch -eq 1 ) || ( $debug_uvc -eq 1 ) ]]; then
	echo -e "\e[32mCompiling media/common modules\e[0m"
	sudo make -j -C $KBASE M=drivers/media/common/videobuf2
fi
...
try_module_install videodev				~/$LINUX_BRANCH-videodev.ko 			/lib/modules/`uname -r`/kernel/drivers/media/v4l2-core/videodev.ko
if [[ ( ${k_maj_min} -ge 500 ) && ( $debug_uvc -eq 1 ) ]]; then
	try_module_install videobuf2-common		~/$LINUX_BRANCH-videobuf2-common.ko 	/lib/modules/`uname -r`/kernel/drivers/media/common/videobuf2/videobuf2-common.ko
fi
try_module_install uvcvideo				~/$LINUX_BRANCH-uvcvideo.ko 			/lib/modules/`uname -r`/kernel/drivers/media/usb/uvc/uvcvideo.ko
try_module_install hid_sensor_accel_3d 	~/$LINUX_BRANCH-hid-sensor-accel-3d.ko 	/lib/modules/`uname -r`/kernel/drivers/iio/accel/hid-sensor-accel-3d.ko
try_module_install hid_sensor_gyro_3d	~/$LINUX_BRANCH-hid-sensor-gyro-3d.ko 	/lib/modules/`uname -r`/kernel/drivers/iio/gyro/hid-sensor-gyro-3d.ko


echo -e "\e[92m\n\e[1mScript has completed. Please reboot for the changes to take effect.\n\e[0m"
